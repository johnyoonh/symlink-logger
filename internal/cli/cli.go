...
func Main() {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "symlog:", err)
		os.Exit(1)
	}
}
...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  symlog mount-all --root ROOT [--recursive] [--replace]")
	fmt.Fprintln(os.Stderr, "  symlog unmount --old OLD [--restore] [--registry PATH]")
	fmt.Fprintln(os.Stderr, "  symlog unmount-all [--registry PATH] [--restore]")
	fmt.Fprintln(os.Stderr, "  symlog restore --old OLD [--registry PATH]")
}
